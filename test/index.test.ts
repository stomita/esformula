...
describe('esformula', () => {
  ...
  test('object expression', () => {
    const [a, b, c] = [1, 2, 3];
    assert.deepEqual(parse('({ a, b, d: c })').evaluate({ a, b, c }), { a, b, d: c });
    const obj = { c: 4, d: true };
    assert.deepEqual(parse('({ a, ...obj })').evaluate({ a, obj }), { a, ...obj });
  });
  ...
  test('logical expression', () => {
    ...
    // @ts-ignore
    assert.ok(parse('a === 0 || a === 1').evaluate({ a }) === (a === 0 || a === 1));
  });
  ...
  test('should avoid prototype access', () => {
    ...
    try {
      formula.evaluate();
      assert.fail('should not be evaluated if constructor contains prototype/constructor access');
    } catch (err) {
      assert.ok(true);
    }
  });
